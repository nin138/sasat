...
const getInParenValues = (tokens: Token[], fromIndex = 0) => {
  ...
  const start = sliced.findIndex(
    it => it.kind === TokenKind.Separator && it.value === '(',
  );
  const end = sliced.findIndex(
    it => it.kind === TokenKind.Separator && it.value === ')',
  );
...
const startStrMap: {
  word: string;
  fn: (str: string, table: SerializedTable) => SerializedTable;
}[] = [
  {
    word: 'PRIMARY KEY',
    fn: (str: string, table) => {
  ...
  },
  {
    ...
    fn: (str: string, table) => {
      ...
      const refIndex = tokens.findIndex(
        it => it.kind === TokenKind.Keyword && it.value === 'REFERENCES',
      );
      ...
      const onDelete = tokens.findIndex(
        it => it.kind === TokenKind.Keyword && it.value === 'ON DELETE',
      );
      ...
      const reference: Reference = {
        ...
        onUpdate:
          onUpdate !== -1
            ? (tokens[onUpdate + 1].value as ForeignKeyReferentialAction)
            : undefined,
        onDelete:
          onDelete !== -1
            ? (tokens[onDelete + 1].value as ForeignKeyReferentialAction)
            : undefined,
      };
      return {
        ...table,
    ...
    fn: (str, table) => {
      ...
      const getDefault = () => {
        const index = tokens.findIndex(
          it => it.kind === TokenKind.Keyword && it.value === 'DEFAULT',
        );
      ...
      const getDefaultValue = (
        ...
        if (!defaultToken || defaultToken.kind === TokenKind.Identifier)
          return undefined;
        if (
          defaultToken.kind === TokenKind.Keyword &&
