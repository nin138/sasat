...
export const generateTsTypeDef = (gql: IrGql) => {
  ...
  gql.types.forEach(type => {
    obj.set(
      type.typeName,
      tsArrayString(
        type.params.map(it => `${it.name}: ${getGqlTypeString(it)}`),
      ),
    );
  });
  obj.set(
  ...
  obj.set(
    ...
    tsArrayString(
      gql.mutations.entities.flatMap(it => {
        ...
        if (it.create)
          mutations.push(
            `create${it.entityName}${createParamString(it.onCreateParams)}: ${
              it.entityName
            }!`,
          );
        if (it.update)
          mutations.push(
            `update${it.entityName}${createParamString(
              it.onUpdateParams,
            )}: Boolean!`,
          );
        if (it.delete)
          mutations.push(
            `delete${it.entityName}${createParamString(
              it.onDeleteParams,
            )}: Boolean!`,
          );
        return mutations;
  ...
  const onCreate = gql.mutations.entities
    ...
    .map(
      it =>
        `${it.entityName}Created${createParam(it.subscription)}: ${
          it.entityName
        }!`,
    );
  const onUpdate = gql.mutations.entities
    ...
    .map(
      it =>
        `${it.entityName}Updated${createParam(it.subscription)}: ${
          it.entityName
        }!`,
    );
  const onDelete = gql.mutations.entities
    ...
    .map(
      it =>
        `${it.entityName}Deleted${createParam(it.subscription)}: Deleted${
          it.entityName
        }!`,
    );
