...
export const makeClassProperties = (node: EntityNode) => {
  ...
  return [
    ...
    tsg
      ...
      .modifiers(tsg.propertyModifiers().readonly())
      .initializer(
        tsg.array(node.fields.map((it) => tsg.string(it.fieldName))),
      ),
    tsg
      .propertyDeclaration(
        "primaryKeys",
    ...
    tsg
      ...
      .initializer(
        tsg.array(
          node.identifyKeys
            .map(
              (it) => node.fields.find((f) => it === f.columnName)!.fieldName,
            )
            .map(tsg.string),
        ),
      ),
...
const makeDefaultValueMethod = (node: EntityNode) => {
  const columns = node.fields.filter(
    (it) =>
      it.column.defaultCurrentTimeStamp || it.column.default !== undefined,
  );
